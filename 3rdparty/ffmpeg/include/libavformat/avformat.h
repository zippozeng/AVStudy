...
#define AVFMT_AVOID_NEG_TS_MAKE_ZERO         2 ///< Shift timestamps so that they start at 0
    ...
    /**
     * Forced video codec.
     * This allows forcing a specific decode, even when there are multiple with
     * the same codec_id.
     * Demuxing: Set by user
     */
 ...
 * If the decode parameter is non-NULL, av_find_best_stream will find the
 * default decode for the stream's codec; streams for which no decode can
 ...
 * @param decoder_ret       if non-NULL, returns the decode for the
 *                          selected stream
 ...
 *          could be found,
 *          AVERROR_DECODER_NOT_FOUND if streams were found but no decode
 *
 ...
 * that what is there are valid frames for the decode. It will split what is
 * stored in the file into frames and return one for each call. It will not
 * omit invalid data between valid frames so as to give the decode the maximum
